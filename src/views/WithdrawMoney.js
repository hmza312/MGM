...
import { withdrawMoney } from "../api/Users";
let total = 0;
let limits = {
  5000: 100,
  1000: 100,
...
const WithdrawMoney = () => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const {
        data: { message },
      } = await postRequest(withdrawMoney(), values, withJWT);
  ...
  return (
    <Row>
      <Col md={1}></Col>
      <Col md={10}>
        <ComponentCard title="Withdraw Money">
          <Row>
            <Form>
              <FormGroup>
                <Label for="exampleEmail">Your Account Balance</Label>
                <Input
                  id="exampleEmail"
                  name="amount"
                  value={amount}
